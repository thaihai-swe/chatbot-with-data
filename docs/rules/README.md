...
Rule files under `docs/rules/` are shipped overwrite-only files that define deterministic agent behavior constraints. Each file must:
...
To add a new rule:
1. Create `docs/rules/<name>.md` with title and rule content.
2. Add `docs/rules/<name>.md` to `manifest.json` `files.overwrite`.
