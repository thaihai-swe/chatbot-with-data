...
* End-to-end ingestion → query → answer works
* Users can upload documents and ask questions against them
* Duplicate documents can be detected during ingestion
* Answers include citations linked to retrieved chunks
* Unsupported questions are refused
* Debug output is visible for all runs
* Multiple retrieval strategies can be compared
* Pre-retrieval query intelligence can expand and decompose queries before retrieval
* Query expansion, query decomposition, HyDE, and synonym expansion can be toggled and evaluated
* Dynamic routing can select retrieval strategies and explain the decision
* Automatic collection detection can select relevant collections when enabled
* Configurable rerankers can be compared
* Parent-child retrieval can expand precise child hits into richer parent context
* Semantic chunking can be compared against fixed-size chunking
* Streaming UI can progressively display generated answers
* Evaluation runs produce pass/fail results
* Experiment dashboard shows metrics and regressions
* Strategy comparison shows measurable differences
* Collections can isolate different knowledge bases
* Configuration can be changed and reused across experiments
* The project clearly demonstrates RAG, safety, observability, and evaluation in a portfolio

Target quality metrics:

* ≥ 80% correct answers or correct refusals
* ≥ 80% Recall@k on evaluation dataset
* ≥ 85% citation accuracy
* ≥ 90% unsupported-question refusal correctness
* ≥ 90% prompt-injection resistance on adversarial test set
* ≥ 80% automatic collection detection accuracy on labeled test cases
* ≥ 80% duplicate detection accuracy on duplicate test cases
* Strategy comparison must show measurable retrieval or answer-quality differences across at least three retrieval strategies
