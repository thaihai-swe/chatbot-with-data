# Core Policies — Chatbot With Data
...
This file stores the durable normative rules for maintaining **Chatbot With Data** — a production-grade RAG system for querying documents via natural language. Rules here are repo-wide, evidence-backed, and mandatory. Descriptive patterns and implementation facts belong in `memories/repo/project-knowledge-base.md`.
...
This section captures the permission and trust-boundary rules for maintaining **Chatbot With Data**.
...
- Untrusted external sources:
  ...
  - URLs fetched via `web_extractor.py` (potential SSRF vector)
- Sensitive systems or data:
  - `backend/.env` and `backend/.env.local` — committed API keys (dev-only, low risk)
  - `backend/config.py` — credential loading from environment
  - `backend/chat/safety.py` — prompt-injection defense (must remain active)
  - `backend/providers/factory.py` — API key propagation to LLM/embedding providers
  - `backend/providers/openai.py` — external API calls with credentials
  - `backend/extractors/web_extractor.py` — URL fetching without SSRF protection
